...
class ResponsiveCRMSupabaseUser extends BaseAuthUser {
  ResponsiveCRMSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> responsiveCRMSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ResponsiveCRMSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
